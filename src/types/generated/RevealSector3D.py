...
class RevealSector3DBoundingBox:
...
class RevealSector3DThreedFile:
...
class RevealSector3D:
    """The bounding box of the subtree with this sector as the root sector. Is null if there are
    no geometries in the subtree.
    """
    bounding_box: RevealSector3DBoundingBox
    ...
    /3d/files/{id} to retrieve the file.
    """
    threed_files: List[RevealSector3DThreedFile]

    def __init__(self, bounding_box: RevealSector3DBoundingBox, depth: int, id: int, parent_id: int, path: str, threed_files: List[RevealSector3DThreedFile]) -> None:
